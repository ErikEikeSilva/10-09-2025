...
-- Limpa se já existir (ordem contrária às FKs)
...
CREATE TABLE produtos (
    ...
    categoria_id INT,
    CONSTRAINT fk_produtos_categoria_id FOREIGN KEY (categoria_id) REFERENCES categorias(id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE enderecos (
    ...
    estado VARCHAR(2),
    cep VARCHAR(10),
    CONSTRAINT fk_enderecos_cliente_id FOREIGN KEY (cliente_id) REFERENCES usuarios(id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE pedidos (
    ...
    status VARCHAR(50),
    total DECIMAL(10,2),
    CONSTRAINT fk_pedidos_cliente_id FOREIGN KEY (cliente_id) REFERENCES usuarios(id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE itens_pedido (
    ...
    CONSTRAINT fk_itens_pedido_pedido_id FOREIGN KEY (pedido_id) REFERENCES pedidos(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_itens_pedido_produto_id FOREIGN KEY (produto_id) REFERENCES produtos(id) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB;
...
-- Inserções (mantive seus dados de exemplo; como as PKs são AUTO_INCREMENT,
-- se preferir deixar o DB gerar ids, remova os valores de id nas INSERTs.)
...
-- Exemplos de SELECTs (mantive alguns dos seus exemplos)
...
SELECT u.nome, e.rua FROM usuarios AS u JOIN enderecos AS e ON u.id = e.cliente_id;
SELECT p.nome AS produto, c.nome AS categoria FROM produtos AS p JOIN categorias AS c ON p.categoria_id = c.id;
SELECT u.nome, p.status FROM usuarios AS u JOIN pedidos AS p ON u.id = p.cliente_id;
SELECT ip.pedido_id, prod.nome AS nome_produto FROM itens_pedido AS ip JOIN produtos AS prod ON ip.produto_id = prod.id;
...
SELECT u.nome FROM usuarios AS u JOIN pedidos AS p ON u.id = p.cliente_id WHERE p.status = 'Enviado';
SELECT p.nome FROM produtos AS p JOIN categorias AS c ON p.categoria_id = c.id WHERE c.id = 3;
SELECT
...
-- Exemplo de transação: inserir pedido, item e ajustar estoque
START TRANSACTION;
INSERT INTO pedidos (id, cliente_id, data_pedido, status, total) VALUES (31, 1, NOW(), 'Pago', 360.00);
INSERT INTO itens_pedido (id, pedido_id, produto_id, quantidade, preco_unitario) VALUES (31, 31, 1, 2, 180.00);
UPDATE produtos SET estoque = estoque - 2 WHERE id = 1;
COMMIT;

-- Cancelamento do pedido com reposição de estoque
START TRANSACTION;
UPDATE pedidos SET status = 'Cancelado' WHERE id = 31;
UPDATE produtos SET estoque = estoque + 2 WHERE id = 1;
COMMIT;

-- Exemplo final: atualizar preço de produtos da categoria 1
START TRANSACTION;
UPDATE produtos
SET preco = 900.00
